...
const CONFIG = {
  name: "Stok",
  storeName: "Stok Online",
  city: "Porto Alegre",
  region: "Porto Alegre",
  source: "vipcommerce:stok",

  organizationId: 130,
  vipcommerceFilialId: 146,
  filialId: 1,
  centroDistribuicaoId: 1,

  domain: "stokonline.com.br",
...
async function getPublicCredentials(): Promise<{
  ...
  console.log(
    "Obtendo configuração pública atual do Stok..."
  );

  const homeResponse = await fetch(
    "https://www.stokonline.com.br",
    {
      headers: {
        Accept: "text/html",
  ...
  if (!homeResponse.ok) {
    throw new Error(
      `Falha ao abrir loja Stok: HTTP ${homeResponse.status}`
    );
  }
  ...
  if (!mainMatch) {
    throw new Error(
      "Bundle main do Stok não encontrado."
    );
  }

  const mainUrl =
    `https://www.stokonline.com.br/${mainMatch[1]}`;
  ...
  if (!mainResponse.ok) {
    throw new Error(
      `Falha ao baixar main Stok: HTTP ${mainResponse.status}`
    );
  }
  ...
  if (chunks.length === 0) {
    throw new Error(
      "Nenhum chunk do Stok encontrado."
    );
  }
  ...
  for (const chunk of chunks) {
    try {
      const response = await fetch(
        `https://www.stokonline.com.br/${chunk}`,
        {
          headers: {
            Accept: "*/*",
            "User-Agent": "BaratoRadar/1.0",
...
async function getStoreToken(): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error(
      `Falha no login Stok: HTTP ${response.status}`
    );
  }
  ...
  if (!body.success || !body.data) {
    throw new Error(
      "JWT público da loja Stok não recebido."
    );
  }
...
const DEPARTMENTS = [
  { id: 8, name: "MERCEARIA ALIMENTOS" },
  { id: 6, name: "BEBIDAS" },
  { id: 3, name: "PRODUTOS DE LIMPEZA" },
  { id: 11, name: "CARNES" },
  { id: 12, name: "FRIOS E LATICÍNIOS" },
  { id: 7, name: "HIGIENE E BELEZA" },
  { id: 2, name: "HORTIFRUTI" },
  { id: 9, name: "CONGELADOS" },
  { id: 1, name: "BAZAR" },
  { id: 10, name: "PRODUTOS SAUDÁVEIS" },
  { id: 4, name: "PET SHOP" },
  { id: 13, name: "PADARIA E CONFEITARIA" },
...
function normalizeProduct(
  ...
  return {
    ...
    url: product.link
      ? `https://www.stokonline.com.br/produto/${product.produto_id}/${product.link}`
      : undefined,
  };
}
...
async function main() {
  ...
  console.log("");
  console.log("=================================");
  console.log(" STOK — CARGA CONCLUÍDA");
  console.log("=================================");
  console.log(
    `Recebidos brutos: ${totalReceived}`
  );
...
main().catch((error) => {
  console.error("");
  console.error("ERRO NO CONNECTOR STOK");
  console.error(error);
  process.exit(1);
});
